...
assert_fail_with_message \
  ...
  bash -c "cd '${ROOT_DIR}' && '${CCR}' deepseek --worktree 'bad/name'"

assert_fail_with_message \
  "reject non git repo" \
  "--worktree requires running inside a git repository" \
  bash -c "cd '${TMP_DIR}' && '${CCR}' deepseek --worktree demo"

echo "All ccr deepseek worktree MVP tests passed."

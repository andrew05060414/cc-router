...
assert_contains \
  "ccr 9router doctor advertises alias rewrite" \
  ...
  env \
    ...
    bash "${ROOT_DIR}/scripts/ccr" 9router doctor

assert_contains \
  "ccr 9router doctor detail shows alias mapping" \
  ...
  env \
    ...
    NINEROUTER_SONNET_MODEL="cc-normal" \
    NINEROUTER_HAIKU_MODEL="cc-lite" \
    bash "${ROOT_DIR}/scripts/ccr" 9router doctor detail

...
source "${ROOT}/scripts/lib/cc-common.sh"
...
assert_false() {
  local label="$1" val="$2"
  if cc_router_falsy "${val}"; then
    printf 'OK  false %s (%s)\n' "${label}" "${val}"
  elif cc_router_truthy "${val}"; then
...
[[ "$(cc_router_parse_bool '' n)" == "false" ]] || { echo "FAIL empty default"; fail=1; }
[[ "$(cc_router_parse_bool '' y)" == "true" ]] || { echo "FAIL empty default y"; fail=1; }

...
cc_remote_settings_local_file() {
  printf '%s\n' "${HOME}/.claude/settings.json"
}

cc_remote_claude_md_local_file() {
  printf '%s\n' "${HOME}/.claude/CLAUDE.md"
}

cc_remote_ssh_config_file() {
  printf '%s\n' "${HOME}/.ssh/config"
}

cc_remote_resolve_host() {
  local input="$1"
  ...
  local ssh_config
  ssh_config="$(cc_remote_ssh_config_file)"
...
cc_remote_ssh_target() {
  local input="$1"
  local host user
  host="$(cc_remote_resolve_host "$input")"
  user="$(cc_remote_resolve_user "$input")"
  printf '%s@%s\n' "$user" "$host"
}
...
cc_remote_pack() {
  ...
  cc_remote_config_generate_silent "$pack_dir/settings.json"

  # 3. 复制 CLAUDE.md
  local claude_md
  claude_md="$(cc_remote_claude_md_local_file)"
  if [[ -f "$claude_md" ]]; then
  ...
  # 4. 复制选中的技能包
  cc_remote_skills_pack "$pack_dir/skills"

  # 5. 生成远程安装脚本
  cc_remote_generate_install_script "$pack_dir"
...
cc_remote_generate_install_script() {
  local pack_dir="$1"
...
cc_remote_setup() {
  local input="$1"
  local target
  target="$(cc_remote_ssh_target "$input")"
  ...
  cc_remote_ssh_exec "$target" "rm -rf ${remote_pack} && mkdir -p ${remote_pack}"
  cc_remote_scp_to "$pack_dir/" "$target" "$remote_pack/"

  # 远程执行安装
  echo "[cc-remote] 远程执行安装..."
  cc_remote_ssh_exec "$target" "bash ${remote_pack}/install-claude-remote.sh"
...
cc_remote_sync() {
  ...
  # 上传到远程
  local remote_pack="/tmp/cc-router-remote-pack"
  cc_remote_ssh_exec "$target" "mkdir -p ${remote_pack}"
  cc_remote_scp_to "$pack_dir/settings.json" "$target" "${remote_pack}/"
  if [[ -f "$pack_dir/CLAUDE.md" ]]; then
    cc_remote_scp_to "$pack_dir/CLAUDE.md" "$target" "${remote_pack}/"
  fi
  if [[ -d "$pack_dir/skills" ]]; then
    cc_remote_ssh_exec "$target" "rm -rf ${remote_pack}/skills"
    cc_remote_scp_to "$pack_dir/skills" "$target" "${remote_pack}/"
  fi

  # 远程应用
  cc_remote_ssh_exec "$target" "
    mkdir -p ~/.claude

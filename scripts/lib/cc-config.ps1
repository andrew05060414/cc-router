...
function Show-CcRouterConfig {
  ...
  Write-Host ''
  if (Test-CcRouterCacheFix9routerEnabled) {
    Write-Host "cacheFix9routerEnabled (effective): enabled → cc -9 uses $(Get-CcRouterCacheFixUrl)/v1 → 9Router at $(Get-CcRouterNineRouterRealUrl)"
  } else {
    Write-Host "cacheFix9routerEnabled (effective): disabled → cc -9 direct to $(Get-CcRouterNineRouterRealUrl)/v1"
  }
  if ($env:CC_CACHE_FIX_9ROUTER_ENABLED) {

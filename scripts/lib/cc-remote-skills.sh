...
cc_remote_skills_root() {
  printf '%s\n' "${HOME}/.claude/skills"
}

cc_remote_skills_list() {
  local root
  root="$(cc_remote_skills_root)"
  if [[ ! -d "$root" ]]; then
    return 0
  fi
...
cc_remote_skills_list_projects() {
...
cc_remote_skills_select() {
  ...
  done < <(cc_remote_skills_list_bundled)
  ...
  done < <(cc_remote_skills_list_projects)
  ...
  for idx in $selection; do
    if [[ "$idx" =~ ^[0-9]+$ ]]; then
      ...
      if (( global_idx >= 0 && global_idx < ${#global_skills[@]} )); then
        ...
        local skill_src="$(cc_remote_skills_root)/$skill_name"
...
cc_remote_skills_pack() {
  ...
  if [[ ! -d "$pack_dir/skills" ]] || [[ -z "$(ls -A "$pack_dir/skills" 2>/dev/null)" ]]; then
    cc_remote_skills_pack_bundled "$pack_dir/skills"
  fi
...
cc_remote_skills_copy_from_project() {
  local project_dir="$1"
  local dest="$2"
